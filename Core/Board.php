...
function GetLastReplyInfo_SubBoard($id)
{
	global $db_prefix, $memsurl, $forumurl, $conn;
	$ret = "";
	$sql = $conn->query("SELECT ID FROM ".$db_prefix."board_topics WHERE subboard='".$id."'");
	if ($sql->num_rows > 0) {
		$data = array();
		while ($row = $sql->fetch_assoc()) {
			$sql2 = $conn->query("SELECT * FROM " . $db_prefix . "board_topicreply WHERE topicid='" . $row['ID'] . "' ORDER BY dateadded DESC LIMIT 1");
			if ($sql2->num_rows > 0) {
		...
		if (count($data) > 0) {
			array_sort_by_column($data, "dateadded");
